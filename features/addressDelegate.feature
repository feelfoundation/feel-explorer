Feature: Delegate page
  Scenario: should show title, summary, and transactions
    Given I'm on page "/address/537318935439898807F"
    Then I should see "537318935439898807F" in "top title" element
    And I should see "Delegate account" in "sub title" element
    And I should see "Home Address 537318935439898807F" in "breadcrumb" element
    And I should see table "summary" containing:
      ...
      | Total balance | 3,288.91044586 F39                                               |
      | Transactions  | 1021 1                                                           |
    ...
    And I should see table "transactions table" with 20 rows starting with:
      ...
      |----------------|---------------------------|-----------------------|--------------|--------------|---------|---------------|
      | /\d{18,20}/    | /2017\/06\/16 \d\d:58:48/ | 16313739661670634666F | genesis_17   | 0.00545 F39  | 0.1 F39 | Confirmed     |
      | /\d{18,20}/    | /2017\/06\/16 \d\d:58:48/ | 16313739661670634666F | genesis_17   | 0.006476 F39 | 0.1 F39 | Confirmed     |
      | /\d{18,20}/    | /2017\/06\/16 \d\d:58:48/ | 16313739661670634666F | genesis_17   | 0.019556 F39 | 0.1 F39 | Confirmed     |
      | /\d{18,20}/    | /2017\/06\/16 \d\d:58:48/ | 16313739661670634666F | genesis_17   | 0.021278 F39 | 0.1 F39 | Confirmed     |
      | /\d{18,20}/    | /2017\/06\/16 \d\d:58:48/ | 16313739661670634666F | genesis_17   | 0.027222 F39 | 0.1 F39 | Confirmed     |
  ...
  Scenario: should allow to show voters
    ...
    Then I should see "16313739661670634666F • gottavoteemall •" in "voters" element
  ...
  Scenario: should allow to show votes
    Given I'm on page "/address/4401082358022424760F"
    And I wait 0.5 seconds
    When I click "details tab"
  ...
  Scenario: should show the total number of voters
    Given I'm on page "/address/11595026565287740051F"
    And I wait 0.5 seconds
  ...
  Scenario: should show Forging statistics
    ...
    | Name          | genesis_42             |
    | Address       | 11595026565287740051F  |
    | Uptime        | 100%                   |
    | Rank / Status | 25 / Active            |
    | Approval      | 99.58%                 |
    | Vote weight   | 99,685,421.108366 F39  |
    | Forged        | 1,172.91881186 F39     |
    | Blocks        | 241 (0 missed)         |
