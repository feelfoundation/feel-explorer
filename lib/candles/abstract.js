...
function AbstractCandles(client) {
	...
	this.retrieveTrades = function (start, end, cb) {
		...
		logger.debug(`Candles: Retrieving trades from ${self.name}...`);

		async.doUntil(
			(next) => {
				logger.debug(`Candles: Start: ${start ? new Date(start * 1000).toISOString() : 'N/A'} End: ${end ? new Date(end * 1000).toISOString() : 'N/A'}`);
				request.get({
					...
					if (!self.validData(data)) {
						logger.warn('Candles:', 'Invalid data received');
						return next();
					}

					if (self.validTrades(results, data)) {
						logger.debug(['Candles:', (start ? start.toString() : 'N/A'), 'to', (end ? end.toString() : 'N/A'), '=> Found', data.length.toString(), 'trades'].join(' '));
			...
			(err) => {
				...
				logger.debug(`Candles: ${results.length.toString()} trades in total retrieved`);
	...
	this.saveCandles = function (results, cb) {
		...
		multi.exec((err, replies) => {
			...
			logger.debug(`Candles: ${replies.length.toString()} candles saved`);
			return cb(null, results);
		});
	};
	...
	this.restoreCandles = function (duration, index, cb) {
		client.LRANGE(self.candleKey(duration), index, -1, (err, reply) => {
			...
			logger.debug(`Candles: ${reply.length.toString()} candles restored`);
			return cb(null, reply);
		});
	};

	const _buildCandles = function (trades, cb) {
		logger.debug(`Candles: Building ${self.duration} candles for ${self.name}...`);
	...
	const _updateCandles = function (trades, cb) {
		logger.debug(`Candles: Updating ${self.duration} candles for ${self.name}...`);
