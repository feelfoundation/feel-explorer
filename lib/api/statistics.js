...
const mapFeelCoreV3StateFormat = (state) => {
	const stateMapping = {
		unknown: 0,
		disconnected: 1,
		connected: 2,
...
module.exports = function (app) {
	...
	this.getBlocks = function (query, error, success) {
		...
		async.doUntil(
			(next) => {
				request.get({
					url: app.get('feel address') + blocks.url(offset, limit),
					json: true,
				}, (err, resp, body) => {
	...
	this.getLastBlock = function (query, error, success) {
		...
		request.get({
			url: app.get('feel address') + blocks.url(0, 1),
			json: true,
		}, (err, res, body) => {
	...
	function Peers(locator) {
		...
		this.process = function (p, next) {
			...
			switch (parseInt(mapFeelCoreV3StateFormat(p.state), 10)) {
			case 1:
	...
	const peersMapper = function (o) {
		...
		return {
			...
			wsPort: o.wsPort || 'n/a',
			state: mapFeelCoreV3StateFormat(o.state),
			os: o.os,
			version: o.version,
	...
	this.getPeers = function (query, error, success) {
		...
		async.doUntil(
			(next) => {
				logger.debug(`Requesting ${app.get('feel address') + peers.url(offset, limit)}`);
				request.get({
					url: app.get('feel address') + peers.url(offset, limit),
					json: true,
				},
				/* eslint-disable consistent-return */
				(err, resp, body) => {
