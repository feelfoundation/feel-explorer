...
module.exports = function (app) {
	this.getBlockHeight = function (error, success) {
		request.get({
			url: `${app.get('feel address')}/node/status`,
			json: true,
		}, (err, response, body) => {
			if (err) {
	...
	this.getBlockByHeight = function (height, error, success) {
		request.get({
			url: `${app.get('feel address')}/blocks?height=${height}`,
			json: true,
		}, (err, response, body) => {
	...
	function getDelegateCall(publicKey, cb) {
		...
		return request.get({
			url: `${app.get('feel address')}/delegates?publicKey=${publicKey}`,
			json: true,
		}, (err, response, body) => {
		...
		this.getBlockHeight(
			...
			(data) => {
				if (data.success === true) {
					return request.get({
						url: `${app.get('feel address')}/blocks?sort=height:desc&limit=20&offset=${blocks.offset(n)}`,
						json: true,
					}, (err, response, body) => {
	...
	function Block() {
		...
		this.getBlock = function (blockId, height, cb) {
			request.get({
				url: `${app.get('feel address')}/blocks?blockId=${blockId}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
				} else if (body && Array.isArray(body.data) && body.data.length === 1) {
	...
	this.getBlockStatus = function (query, error, success) {
		...
		function getConstants(cb) {
			request.get({
				url: `${app.get('feel address')}/node/constants`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return error({ success: false, error: (err || 'Response was unsuccessful') });
				} else if (body && body.data) {
					return cb(null, { ...body.data, nethash: body.data.networkId || body.data.nethash });
				}
				return error({ success: false, error: body.error });
			});
		}
