...
module.exports = function (app) {
	...
	function Account() {
		...
		this.getVotesCall = (publicKey, limit, offset, cb) => {
			request.get({
				url: `${app.get('feel address')}/votes?publicKey=${publicKey}&limit=${limit}&offset=${offset}`,
				json: true,
			}, (err, response, body) => {
		...
		this.getVotersCall = (publicKey, limit, offset, cb) => {
			request.get({
				url: `${app.get('feel address')}/voters?publicKey=${publicKey}&limit=${limit}&offset=${offset}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getAccount = (params, cb) => {
			...
			return request.get({
				url: `${app.get('feel address')}/accounts?${query}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getMultisignaturesGroups = (account, cb) => {
			request.get({
				url: `${app.get('feel address')}/accounts/${coreUtils.parseAddress(account.address)}/multisignature_groups`,
				json: true,
			}, (err, response, body) => {
		...
		this.getMultisignaturesMemberships = (account, cb) => {
			request.get({
				url: `${app.get('feel address')}/accounts/${account.address}/multisignature_memberships`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getIncomingTxsCnt = (account, cb) => {
			...
			return request.get({
				url: `${app.get('feel address')}/transactions?recipientId=${coreUtils.parseAddress(account.address)}&limit=1`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getOutgoingTxsCnt = (account, cb) => {
			...
			return request.get({
				url: `${app.get('feel address')}/transactions?senderId=${coreUtils.parseAddress(account.address)}&limit=1`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getForged = (account, cb) => {
			...
			return request.get({
				url: `${app.get('feel address')}/delegates/${coreUtils.parseAddress(account.address)}/forging_statistics`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
	...
	this.getTopAccounts = (query, error, success) => {
		...
		return request.get({
			url: `${app.get('feel address')}/accounts/?sort=balance:desc&offset=${param(query.offset, 0)}&limit=${param(query.limit, 100)}`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
				return error({ success: false, error: (err || 'Response was unsuccessful') });
