...
function BittrexOrders(...rest) {
	...
	this.url = 'https://bittrex.com/api/v1.1/public/getorderbook?market=BTC-F39&type=both&depth=50';
	this.limit = 100;
	...
	this.retrieveOrders = function (cb) {
		...
		request.get({
			...
			const validateResult = (obj) => {
				if (typeof obj !== 'object') return false;
				if (typeof obj.result !== 'object') return false;
				if (!Array.isArray(obj.result.buy)) return false;
				if (!Array.isArray(obj.result.sell)) return false;
				return true;
			};
