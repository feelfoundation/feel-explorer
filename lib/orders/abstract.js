...
function AbstractOrders(client) {
	...
	this.retrieveOrders = function (cb) {
		logger.debug('Orders:', 'Retrieving orders from', `${self.name}...`);
		...
		request.get({
			...
			const validateResult = (result) => {
				...
				if (!Array.isArray(result.asks)) return false;
				if (!Array.isArray(result.bids)) return false;
				return true;
			};
	...
	this.saveOrders = function (orders, cb) {
		...
		multi.exec((err) => {
			...
			logger.debug('Orders:', 'Orders saved successfully');
	...
	this.restoreOrders = function (cb) {
		client.GET(self.key, (err, reply) => {
			...
			logger.debug('Orders:', 'Orders restored successfully');
			return cb(null, reply);
		});
	};

	this.updateOrders = function (cb) {
		logger.debug('Orders:', 'Updating orders for', `${self.name}...`);
